...
class TestDiscoverCliPath(unittest.TestCase):
    ...
    def _discover(self, home: Path, nvm_bin: str | None = None) -> Path:
        """Run discovery with a controlled home directory and NVM_BIN."""
        env = {'NVM_BIN': nvm_bin} if nvm_bin else {}
        with patch('usage_monitor_for_claude.claude_cli.Path.home', return_value=home), \
             patch.dict(os.environ, env, clear=False):
            if not nvm_bin:
                os.environ.pop('NVM_BIN', None)
            return claude_cli._discover_cli_path()
    ...
    def test_falls_back_to_local_bin(self, _mock_which):
        """When which finds nothing, the native installer location is used."""
        with TemporaryDirectory() as tmp:
            home = Path(tmp)
            binary = home / '.local' / 'bin' / 'claude'
            binary.parent.mkdir(parents=True)
            binary.touch()
            self.assertEqual(self._discover(home), binary)

    @patch('usage_monitor_for_claude.claude_cli.shutil.which', return_value=None)
    def test_falls_back_to_claude_local(self, _mock_which):
        """~/.claude/local/claude is found when ~/.local/bin has no binary."""
        with TemporaryDirectory() as tmp:
            home = Path(tmp)
            binary = home / '.claude' / 'local' / 'claude'
            binary.parent.mkdir(parents=True)
            binary.touch()
            self.assertEqual(self._discover(home), binary)

    @patch('usage_monitor_for_claude.claude_cli.shutil.which', return_value=None)
    def test_local_bin_preferred_over_claude_local(self, _mock_which):
        """~/.local/bin wins when both user locations exist."""
        with TemporaryDirectory() as tmp:
            home = Path(tmp)
            local_bin = home / '.local' / 'bin' / 'claude'
            local_bin.parent.mkdir(parents=True)
            local_bin.touch()
            claude_local = home / '.claude' / 'local' / 'claude'
            claude_local.parent.mkdir(parents=True)
            claude_local.touch()
            self.assertEqual(self._discover(home), local_bin)

    @patch('usage_monitor_for_claude.claude_cli.shutil.which', return_value=None)
    def test_falls_back_to_npm_global_prefix(self, _mock_which):
        """A user-owned npm prefix (~/.npm-global/bin) is checked."""
        with TemporaryDirectory() as tmp:
            home = Path(tmp)
    ...
    @patch('usage_monitor_for_claude.claude_cli.shutil.which', return_value=None)
    def test_falls_back_to_nvm_bin(self, _mock_which):
        """$NVM_BIN/claude is checked for nvm-managed Node installs."""
        with TemporaryDirectory() as tmp, TemporaryDirectory() as nvm_tmp:
            home = Path(tmp)
            binary = Path(nvm_tmp) / 'claude'
            binary.touch()
            self.assertEqual(self._discover(home, nvm_bin=nvm_tmp), binary)
    ...
    def test_last_resort_returns_default_when_nothing_found(self, _mock_which):
        ...
        with TemporaryDirectory() as tmp:
            home = Path(tmp)
            result = self._discover(home)
            self.assertEqual(result, home / '.local' / 'bin' / 'claude')
...
class TestFindInstallations(unittest.TestCase):
    ...
    def test_vscode_extension(self, mock_cli_path, _mock_version):
        ...
        with TemporaryDirectory() as tmp:
            ext_dir = Path(tmp)
            (ext_dir / 'anthropic.claude-code-2.1.69-linux-x64').mkdir()
    ...
    def test_picks_highest_version(self, mock_cli_path, _mock_version):
        ...
        with TemporaryDirectory() as tmp:
            ext_dir = Path(tmp)
            (ext_dir / 'anthropic.claude-code-2.1.63-linux-x64').mkdir()
            (ext_dir / 'anthropic.claude-code-2.1.69-linux-x64').mkdir()
            (ext_dir / 'anthropic.claude-code-2.1.66-linux-x64').mkdir()
            with patch('usage_monitor_for_claude.claude_cli._EXTENSION_DIRS', [('VS Code', ext_dir)]):
                result = find_installations()
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].version, '2.1.69')
    ...
    def test_cli_and_extensions_combined(self, mock_cli_path, _mock_version):
        ...
        with TemporaryDirectory() as tmp:
            ext_dir = Path(tmp)
            (ext_dir / 'anthropic.claude-code-2.1.68-linux-x64').mkdir()
            with patch('usage_monitor_for_claude.claude_cli._EXTENSION_DIRS', [('VS Code', ext_dir)]):
                result = find_installations()
...
class TestRefreshTokenIgnoresCliCommand(unittest.TestCase):
    ...
    def test_uses_native_binary_despite_cli_command(self, mock_path, mock_run):
        ...
        mock_path.is_file.return_value = True
        mock_path.__str__.return_value = '/home/user/.local/bin/claude'
        mock_run.return_value = MagicMock(stdout='Claude Code is up to date (2.1.177)', stderr='', returncode=0)
        result = refresh_token()
        self.assertTrue(result.success)
        self.assertEqual(mock_run.call_args[0][0], ['/home/user/.local/bin/claude', 'update'])
...
class TestRunCli(unittest.TestCase):
    ...
    The Claude CLI writes UTF-8 whatever the ambient locale is.  Letting
    subprocess decode it with the locale codec instead is what caused the
    crash in issue #80 (a codec that cannot decode UTF-8 loses the
    stream), so these tests run real child processes rather than mock the
    mechanism they are about.
    """
