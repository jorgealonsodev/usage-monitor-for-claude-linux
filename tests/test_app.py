...
from usage_monitor_for_claude.tray import SEPARATOR, MenuItem
...
def _make_app(thresholds: list[float] | None = None) -> UsageMonitorForClaude:
    ...
    if thresholds is None:
        thresholds = [80, 95]
    with patch('usage_monitor_for_claude.app.TrayIcon'), \
...
class TestTrayMenuModel(unittest.TestCase):
    """Tests for the menu structure handed to the tray adapter.

    AppIndicator has no click events, so the interaction model is: the
    popup is the first menu entry and the middle-click default; the
    double-click command gets a dedicated entry only when configured.
    """

    def _flatten(self, items):
        return [item for item in items if item is not SEPARATOR]

    def test_show_popup_is_first_and_default(self):
        """The popup entry comes first and is the middle-click default action."""
        app = _make_app()
        try:
            items = app._build_menu_items()
            first = items[0]
            self.assertIsInstance(first, MenuItem)
            self.assertEqual(first.callback, app.on_show_popup)
            self.assertTrue(first.default)
            self.assertTrue(all(not item.default for item in self._flatten(items[1:])))
        finally:
            _cleanup(app)

    def test_no_double_click_entry_without_command(self):
        """Without a configured command there is no dedicated double-click entry."""
        app = _make_app()
        try:
            with patch('usage_monitor_for_claude.app.ON_DOUBLE_CLICK_COMMAND', []):
                items = app._build_menu_items()
            callbacks = [item.callback for item in self._flatten(items)]
            self.assertNotIn(app.on_double_click, callbacks)
        finally:
            _cleanup(app)

    def test_double_click_entry_present_when_configured(self):
        """With a configured command the dedicated entry follows the popup entry."""
        app = _make_app()
        try:
            with patch('usage_monitor_for_claude.app.ON_DOUBLE_CLICK_COMMAND', ['run.sh']):
                items = app._build_menu_items()
            self.assertEqual(items[1].callback, app.on_double_click)
            self.assertFalse(items[1].default)
        finally:
            _cleanup(app)

    def test_menu_contains_quit_and_restart(self):
        """Lifecycle entries survive the port."""
        app = _make_app()
        try:
            callbacks = [item.callback for item in self._flatten(app._build_menu_items())]
            self.assertIn(app.on_quit, callbacks)
            self.assertIn(app.on_restart, callbacks)
        finally:
            _cleanup(app)

    def test_test_commands_submenu_disabled_without_commands(self):
        """The test-commands submenu is insensitive when no event command is set."""
        app = _make_app()
        try:
            with patch('usage_monitor_for_claude.app.ON_RESET_COMMAND', []), \
                 patch('usage_monitor_for_claude.app.ON_STARTUP_COMMAND', []), \
                 patch('usage_monitor_for_claude.app.ON_THRESHOLD_COMMAND', []), \
                 patch('usage_monitor_for_claude.app.ON_DOUBLE_CLICK_COMMAND', []):
                items = app._build_menu_items()
            submenu_items = [item for item in self._flatten(items) if item.submenu is not None]
            self.assertEqual(len(submenu_items), 1)
            self.assertFalse(submenu_items[0].enabled)
        finally:
            _cleanup(app)

    def test_on_double_click_runs_command(self):
        """The dedicated menu entry runs the double-click command with live data."""
        app = _make_app()
        try:
            with patch.object(app, '_run_double_click_command') as mock_cmd:
                app.on_double_click()
            mock_cmd.assert_called_once()
        finally:
            _cleanup(app)
