...
class TestRunEventCommand(unittest.TestCase):
    ...
    @patch('usage_monitor_for_claude.command.subprocess.Popen')
    def test_starts_new_session(self, mock_popen: MagicMock):
        """The command is detached into its own session so it survives the monitor."""
        run_event_command(['test'], {'USAGE_MONITOR_EVENT': 'reset'})
    ...
    def test_cwd_set_to_executable_dir_when_frozen(self, mock_sys: MagicMock, mock_popen: MagicMock):
        ...
        mock_sys.executable = '/opt/usage-monitor/app'

        run_event_command(['test'], {'USAGE_MONITOR_EVENT': 'reset'})

        kwargs = mock_popen.call_args[1]
        self.assertEqual(kwargs['cwd'], Path('/opt/usage-monitor'))
...
class TestRunEventCommandCaptureOutput(unittest.TestCase):
    ...
    @patch('usage_monitor_for_claude.command.dialogs')
    @patch('builtins.print')
    @patch('usage_monitor_for_claude.command.threading.Thread')
    @patch('usage_monitor_for_claude.command.subprocess.Popen')
    def test_capture_output_prints_streams_and_exit_code(self, mock_popen: MagicMock, mock_thread: MagicMock, mock_print: MagicMock, _dialogs: MagicMock):
    ...
    @patch('usage_monitor_for_claude.command.threading.Thread')
    @patch('usage_monitor_for_claude.command.subprocess.Popen')
    def test_capture_output_reports_empty_streams(self, mock_popen: MagicMock, mock_thread: MagicMock, mock_print: MagicMock, _dialogs: MagicMock):
    ...
    @patch('usage_monitor_for_claude.command.threading.Thread')
    @patch('usage_monitor_for_claude.command.subprocess.Popen')
    def test_capture_output_shows_error_box_on_nonzero_exit(self, mock_popen: MagicMock, mock_thread: MagicMock, mock_print: MagicMock, mock_dialogs: MagicMock):
        ...
        box_message = mock_dialogs.show_error.call_args[0][1]
    ...
    @patch('usage_monitor_for_claude.command.threading.Thread')
    @patch('usage_monitor_for_claude.command.subprocess.Popen')
    def test_capture_output_no_error_box_on_success(self, mock_popen: MagicMock, mock_thread: MagicMock, mock_print: MagicMock, mock_dialogs: MagicMock):
...
class TestStartupFailureWindow(unittest.TestCase):
    ...
    def _run(self, returncode: int, runtime: float, report_late_failures: bool):
        ...
        Returns the show_error mock and everything the command printed.
        """
        ...
        with (
            patch('usage_monitor_for_claude.command.dialogs') as mock_dialogs,
        ...
        return mock_dialogs.show_error, printed
