...
The popup lists the Claude Code version of the native CLI and of each IDE extension it finds. Installs it cannot see - for example one inside a container, under another user, or on a remote host - are missing from that list. Use `cli_command` to have their versions reported as well.
...
| Key | Default | Description |
|-----|---------|-------------|
| `cli_command` | *(none)* | Object mapping a display name to a base command (array of strings) whose Claude Code version is reported alongside the auto-detected ones, e.g. an install inside a container or on a remote host |

```json
{
    "cli_command": {
        "Devbox": ["ssh", "devbox", "claude"]
    }
}
```

An entry only appears once its command reports a version, so if it stays missing, run the command yourself in a terminal - `ssh devbox claude --version` has to print a version number.

- **This setting is display only.** Automatic token refresh keeps using the native CLI, whose credentials this app reads.
- **The version is read once per app start.** After updating a Claude Code listed this way, restart the app to see the new version.
...
The detail popup can be dragged anywhere by holding the mouse down on its header bar - no pinning required - and can be pinned open (pin button in the header) so it stays visible when it loses focus. The position you drag it to is remembered: the next time the popup opens it appears there again (falling back to the usual corner placement when that position is no longer on any screen). The remembered coordinates are stored in `~/.config/usage-monitor-for-claude/state*.json` (respects `$XDG_CONFIG_HOME`; instances started with `--config-dir` get their own suffixed file). Use `compact_hide` to strip the pinned popup down to just the usage bars you care about - the entries listed here are hidden **only while the popup is pinned**, and reappear when you unpin it.
...
| `on_double_click_command` | *(none)* | Shell command (or array of commands) run on demand from a dedicated tray menu entry (the AppIndicator protocol has no double-click gesture); the entry appears only while this command is configured |
...
By default, reset times follow your system locale's clock format (whether the locale's time format renders an AM/PM marker decides between 12-hour and 24-hour), so no configuration is needed. Set this key to override the auto-detected format.

| Key | Default | Description |
|-----|---------|-------------|
| `time_format` | *(auto-detected from the system locale)* | Clock format for reset times: `"24h"` (e.g. `14:30`) or `"12h"` (e.g. `2:30 PM`) |
...
Override individual channels as RGBA arrays `[R, G, B, A]` (0-255). Unspecified keys keep their defaults. Which set is used follows your GTK theme: a dark theme (`gtk-application-prefer-dark-theme`, or a theme name containing "dark") gets the light icons, a light theme the dark icons, switching live when the theme changes.

| Key | Default | Description |
|-----|---------|-------------|
| `icon_light` | `{"fg": [255,255,255,255], "fg_half": [255,255,255,80], "fg_dim": [255,255,255,140], "fg_warn": [224,80,80,255]}` | Light icons for a dark panel/theme |
| `icon_dark` | `{"fg": [0,0,0,255], "fg_half": [0,0,0,80], "fg_dim": [0,0,0,140], "fg_warn": [224,80,80,255]}` | Dark icons for a light panel/theme |

## Tray icon color levels

Optionally recolor the tray icon by usage, like a traffic light. `icon_color_levels` is a list of `[threshold, color]` pairs: for each usage percentage, the active color is the pair with the highest threshold at or below it (a percentage below the lowest threshold keeps the normal foreground color).

| Key | Default | Description |
|-----|---------|-------------|
| `icon_color_levels` | *(not set)* | List of `[threshold, color]` pairs recoloring the icon by usage. Threshold: number 0-100. Color: a `"#rgb"`, `"#rrggbb"`, or `"#rrggbbaa"` hex string, or an `[R, G, B, A]` array (0-255) like the `icon_light`/`icon_dark` values |

```json
{
    "icon_color_levels": [[0, "#4caf50"], [70, "#ffb300"], [90, "#e05050"]]
}
```

When set:

- Each usage **bar**'s fill is tinted with the active color for that bar's own percentage (`:overage` bars use the same usage percentage). The percentage **number** follows its field's percentage too - in the `"numbers"` style each of the two stacked numbers is colored independently.
- The levels **take precedence over the warning color**: the `fg_warn` ahead-of-time fill switch is not used while levels are configured (the levels are a superset of that signal).
- Everything else keeps its base colors: the half-tone bar background, the reset-time marker, and the error/status glyphs (`!`, `C!`, `✕`, `$`, `C`).
- The same colors apply on light and dark panels - configured colors are absolute, unlike `icon_light`/`icon_dark`.

When not set (the default), the icon renders exactly as before: plain foreground fills that switch to `fg_warn` when usage outpaces the elapsed time.

To color the popup's bars the same way, see [`bar_color_levels`](#popup-bar-color-levels).
...
## Popup bar color levels

Optionally recolor the popup's usage bars by usage, like a traffic light - the popup counterpart of [`icon_color_levels`](#tray-icon-color-levels). `bar_color_levels` is a list of `[threshold, color]` pairs: each bar (including the Extra Usage bar) takes the color of the pair with the highest threshold at or below **that bar's own percentage**; a percentage below the lowest threshold keeps the normal `bar_fg`.

| Key | Default | Description |
|-----|---------|-------------|
| `bar_color_levels` | *(not set)* | List of `[threshold, color]` pairs recoloring the popup usage bars by usage. Threshold: number 0-100. Color: a `"#rgb"`, `"#rrggbb"`, or `"#rrggbbaa"` hex string, or an `[R, G, B, A]` array (0-255) |

```json
{
    "bar_color_levels": [[0, "#4caf50"], [70, "#ffb300"], [90, "#e05050"]]
}
```

When set, the levels **take precedence over the warning color**: the `bar_fg_warn` ahead-of-time fill switch is not used (the levels are a superset of that signal). The bar background, time dividers, and the time marker keep their base colors. When not set (the default), the bars render exactly as before: `bar_fg` fills that switch to `bar_fg_warn` when usage outpaces the elapsed time.

Set both `bar_color_levels` and [`icon_color_levels`](#tray-icon-color-levels) to the same pairs to color the popup and the tray icon consistently.
