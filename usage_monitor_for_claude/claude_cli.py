...
def _discover_cli_path() -> Path:
    ...
    Strategy
    --------
    1. ``shutil.which('claude')`` - respects PATH.
    2. Common install locations that may not be on the service PATH
       (autostart runs before shell profiles): the native installer
       (``~/.local/bin``, ``~/.claude/local``), a user-owned npm prefix
       (``~/.npm-global/bin``), system-wide npm (``/usr/local/bin``,
       ``/usr/bin``), and the active nvm version (``$NVM_BIN``).
       ``npm prefix -g`` would be authoritative but is far too slow to
       run at import time.
    3. Last resort: return the native installer path so callers'
    ...
    if nvm_bin:
        candidates.append(Path(nvm_bin) / 'claude')
...
PROJECT_URL = 'https://github.com/jorgealonsodev/usage-monitor-for-claude-linux'
        ...
        try:
            ...
            for entry in ext_dir.iterdir():
                if not entry.name.startswith(_EXTENSION_PREFIX):
                    continue
                # Directory name format: anthropic.claude-code-X.Y.Z-linux-x64
...
def _run_cli(command: list[str], timeout: int) -> subprocess.CompletedProcess[str]:
    ...
    ambient locale is, so the codec is pinned here instead of inherited
    from the environment.  Decoding UTF-8 with a locale codec that cannot
    represent it (e.g. a C/POSIX locale on a minimal system) would raise
    inside subprocess's pipe draining and lose the output.
