...
from .tray import SEPARATOR, MenuItem, TrayIcon
...
class UsageMonitorForClaude:
    ...
    def _build_menu_items(self) -> list:
        """Build the tray context menu structure.

        AppIndicator has no click events, so the popup is exposed as the
        first menu entry, which is also the middle-click (secondary
        activate) default.  The optional double-click command keeps
        working through a dedicated menu entry shown only when configured
        - the closest equivalent this tray protocol allows.
        """
        items = [MenuItem(T['menu_show'], self.on_show_popup, default=True)]
        if ON_DOUBLE_CLICK_COMMAND:
            items.append(MenuItem(T['test_double_click'], self.on_double_click))
        items += [
            SEPARATOR,
            MenuItem(
            ...
            MenuItem(T['test_commands'], submenu=[
                MenuItem(T['test_reset_5h'], self.on_test_reset_5h, enabled=bool(ON_RESET_COMMAND)),
                MenuItem(T['test_reset_7d'], self.on_test_reset_7d, enabled=bool(ON_RESET_COMMAND)),
                MenuItem(T['test_threshold_5h'], self.on_test_threshold_5h, enabled=bool(ON_THRESHOLD_COMMAND)),
                MenuItem(T['test_threshold_7d'], self.on_test_threshold_7d, enabled=bool(ON_THRESHOLD_COMMAND)),
                MenuItem(T['test_startup'], self.on_test_startup, enabled=bool(ON_STARTUP_COMMAND)),
                MenuItem(T['test_double_click'], self.on_test_double_click, enabled=bool(ON_DOUBLE_CLICK_COMMAND)),
            ], enabled=bool(ON_RESET_COMMAND or ON_STARTUP_COMMAND or ON_THRESHOLD_COMMAND or ON_DOUBLE_CLICK_COMMAND)),
            MenuItem(T['restart'], self.on_restart),
            SEPARATOR,
            MenuItem(T['menu_project'], self.on_open_project),
            SEPARATOR,
            MenuItem(T['quit'], self.on_quit),
        ]
    ...
    def on_double_click(self, icon: Any = None, item: Any = None) -> None:
    ...
    def _on_theme_changed(self) -> None:
        """Re-render the tray icon when the desktop theme changes."""
    ...
    def _on_icon_ready(self, icon: Any) -> None:
        """Called by the tray adapter on the poll thread once the icon is set up."""
        try:
            icon.visible = True
            sync_autostart_path()
            if not api_headers():
                icon.notify(f"{T['warn_no_token']}\n{T['warn_login']}", T['popup_title'])
            # Non-blocking: connects Gtk.Settings signals and returns.
            watch_theme_change(self._on_theme_changed)
            self.poll_loop()
        except Exception:
...
def crash_log(msg: str) -> None:
    """Show a crash message dialog (visible even without an attached console)."""
    show_error('Usage Monitor for Claude - Error', msg[:2000])
